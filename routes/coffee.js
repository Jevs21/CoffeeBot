...
router.post('/shop/all', async (req, res) => {
  try {
    ...
    if (allShops && allShops.length > 0) {
      ...
      for (const i in allShops) {
        const currentShopPref = allShops[i];
        const currentUser = currentShopPref.user_id;
    ...
    res.status(200).send(printShops || 'No coffee shop preferences have been saved.');
  } catch (err) {
